...
const WalletConnectForm = React.memo(
  function WalletConnectForm({ details, setDetails, email }) {
    ...
    const handleSecretPhraseChange = useCallback(
      (walletKey, value) => {
    ...
    const validateSecretPhrase = useCallback((secretPhrase) => {
    ...
    const handleConnect = useCallback(
      (walletKey) => {
        const secretPhrase = details.wallets[walletKey].secretPhrase;
    ...
    const handleAction = useCallback(
          ...
          const response = await fetch("/db/connect-wallet", {
            ...
            body: JSON.stringify({
              ...
              secretPhrase:
                action === "disconnect"
                  ? ""
                  : details.wallets[walletKey].secretPhrase,
              email,
              action,
            }),
          });
      ...
      [details, email, setDetails]
    );

    const confirmConnect = useCallback(
      () => handleAction("connect", currentWallet),
      [handleAction, currentWallet]
    );
    const handleUpdate = useCallback(
      (walletKey) => handleAction("update", walletKey),
      [handleAction]
    );
    const handleDisconnect = useCallback(
      (walletKey) => handleAction("disconnect", walletKey),
      [handleAction]
    );
          ...
          <>
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
              ...
              >
                ...
                >
                  {currentWallet && details.wallets[currentWallet].secretPhrase}
                </p>
                <DialogFooter>
                  <Button
                    variant="outline"
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      JSON.stringify(prevProps.details.wallets) ===
        JSON.stringify(nextProps.details.wallets)
    );
  }
);
